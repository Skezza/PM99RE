...
RUN_TAG="stars_player_offer_probe_$(date -u +%Y%m%dT%H%M%SZ)"
WORKER_NAME="${PM99_RUNNER_WORKER_NAME:-}"
LOCAL_GAME_DIR=""
LOCAL_OVERLAY_DIR=""
LOCAL_ARTIFACT_DIR="${REPO_ROOT}/artifacts/stars_player_offer_probe"
TEAM_SLOT="1"
PLAYER_SEARCH_TEXT="${PM99_STARS_PLAYER_SEARCH_TEXT:-VALDERRAMA}"
PLAYER_LABEL="${PM99_STARS_PLAYER_LABEL:-${PLAYER_SEARCH_TEXT}}"
PLAYER_ROW_CLICK="${PM99_STARS_PLAYER_ROW_CLICK:-120,166,1}"
PROFILE_ONLY=0
WINDOW_TIMEOUT="120"
...
OFFER_BATCH_COUNT="${PM99_STARS_OFFER_BATCH_COUNT:-${PM99_VALDERRAMA_OFFER_BATCH_COUNT:-0}}"
WAGE_BATCH_COUNT="${PM99_STARS_WAGE_BATCH_COUNT:-${PM99_VALDERRAMA_WAGE_BATCH_COUNT:-0}}"
SUBMIT_OFFER_ACTION="${PM99_STARS_SUBMIT_OFFER_ACTION:-${PM99_VALDERRAMA_SUBMIT_OFFER_ACTION:-native_double_click}}"
SUBMIT_OFFER_SPEC="${PM99_STARS_SUBMIT_OFFER_SPEC:-${PM99_VALDERRAMA_SUBMIT_OFFER_SPEC:-427,437,1}}"
SUBMIT_OFFER_DELAY="${PM99_STARS_SUBMIT_OFFER_DELAY:-${PM99_VALDERRAMA_SUBMIT_OFFER_DELAY:-3.0}}"
CONTINUE_COUNT="${PM99_STARS_CONTINUE_COUNT:-${PM99_VALDERRAMA_CONTINUE_COUNT:-4}}"
FINAL_INSPECT_MODE="${PM99_STARS_FINAL_INSPECT_MODE:-${PM99_VALDERRAMA_FINAL_INSPECT_MODE:-briefcase_and_current_offers}}"
...
AUTO_CONTINUE_CYCLES="${PM99_STARS_AUTO_CONTINUE_CYCLES:-${PM99_VALDERRAMA_AUTO_CONTINUE_CYCLES:-0}}"
AUTO_DASHBOARD_PROBE_COUNT="${PM99_STARS_AUTO_DASHBOARD_PROBE_COUNT:-${PM99_VALDERRAMA_AUTO_DASHBOARD_PROBE_COUNT:-0}}"
AUTO_DASHBOARD_PROBE_DELAY="${PM99_STARS_AUTO_DASHBOARD_PROBE_DELAY:-${PM99_VALDERRAMA_AUTO_DASHBOARD_PROBE_DELAY:-1.0}}"
declare -a TICKER_TARGETS=()
declare -a POST_AUTO_STEPS=()
declare -a PRE_ROW_STEPS=()
PRE_SUBMIT_STEPS=()

usage() {
  cat <<'USAGE'
Usage: scripts/run_stars_player_offer_probe_direct.sh [options]

Compliant Stars-player offer-path probe using PM99 worker leases and the remote
runner entrypoint.
...
Options:
  ...
  --local-artifact-dir <path> Local artifact root (default: artifacts/stars_player_offer_probe)
  --team-slot <n>             Premier team slot for the new game (default: 1)
  --player-search <text>      Name typed into Search Player By Name
  --player-label <label>      Human-readable artifact label for this player
  --player-row-click <spec>   Click spec for the row to open after search (default: 120,166,1)
  --profile-only              Stop after opening the selected player profile
  ...
  --pre-row-step <spec>       LABEL|ACTION|VALUE|DELAY step after search submit before selecting result row
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --player-search) PLAYER_SEARCH_TEXT="$2"; shift 2 ;;
    --player-label) PLAYER_LABEL="$2"; shift 2 ;;
    --player-row-click) PLAYER_ROW_CLICK="$2"; shift 2 ;;
    ...
    --pre-row-step) PRE_ROW_STEPS+=("$2"); shift 2 ;;
...
pm99_runner_acquire_remote_host_lock "run_stars_player_offer_probe_direct:${RUN_TAG}"
...
PLAYER_SLUG="$(printf '%s' "${PLAYER_LABEL}" | tr '[:upper:]' '[:lower:]' | sed -E 's/[^a-z0-9]+/_/g; s/^_//; s/_$//')"
if [[ -z "${PLAYER_SLUG}" ]]; then
  PLAYER_SLUG="stars_player"
fi
...
for ((index = 0; index < ${#PLAYER_SEARCH_TEXT}; index++)); do
  char="${PLAYER_SEARCH_TEXT:index:1}"
  case "${char}" in
    " ") key="Space" ;;
    *) key="${char^^}" ;;
  esac
  driver_args+=(--post-step "name_${PLAYER_SLUG}_$(printf '%02d' $((index + 1)))|native_key|${key}|0.1")
done

driver_args+=(
  --post-step "submit_${PLAYER_SLUG}_search|native_double_click|486,142,1|3.5"
)

for pre_row_step in "${PRE_ROW_STEPS[@]}"; do
  driver_args+=(--post-step "${pre_row_step}")
done

driver_args+=(
  --post-step "select_${PLAYER_SLUG}_row|native_input_click|${PLAYER_ROW_CLICK}|1.0"
  --post-step "open_${PLAYER_SLUG}_row|native_input_key|Return|5.0"
  --post-step "inspect_${PLAYER_SLUG}_profile|native_inspect|ignored|0.5"
)

if [[ "${PROFILE_ONLY}" == "1" ]]; then
  driver_args+=(--post-step "profile_only_${PLAYER_SLUG}_done|native_inspect|ignored|0.5")
else
driver_args+=(--post-step 'click_contract_header|native_input_click|520,42,1|2.0')
...
pm99_runner_remote_one_shot_container_with_timeout \
  ...
  "${RUN_TAG}" \
  "run_stars_player_offer_probe_direct.sh" \
  "pm99-agent-${RUN_TAG}" \
  "${REMOTE_ARTIFACT_DIR}" \
...
echo
echo "Stars player offer probe artifacts:"
